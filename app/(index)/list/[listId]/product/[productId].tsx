...
function ProductContent({ listId, productId }: { listId: string; productId: string }) {
  ...
  return (
    <BodyScrollView
      ...
      <FieldItem label="Created at" value={createdAt ? new Date(createdAt).toDateString() : 'unknown'} />
      <FieldItem label="Quantity" value={quantity.toString()} onChangeText={(value) => setQuantity(Number(value))} />
