...
export default function ShoppingListProductItem({ listId, productId }: { listId: string; productId: string }) {
  ...
  const RightAction = (prog: SharedValue<number>, drag: SharedValue<number>) => {
    ...
    return (
      <Pressable onPressIn={deleteCallback}>
    ...
    <ReanimatedSwipeable
      ...
      <View
        ...
        <Pressable
          ...
          <IconSymbol name={isPurchased ? 'checkmark.circle.fill' : 'circle'} size={28} color={color} />
        </Pressable>
        <Pressable
          onPressIn={() => {
            router.push({
